"""
26 PL2 SCALE BLEND GTR
파이프라인: FE → 4모델(CB/LGB × log/raw) → 신뢰도 필터 PL(50%) → 4모델 재학습 → 가중 평균(원본60%) → 구별 트렌드 보정
변경점: 전략24(PL2) + 전략25(Scale Blending) 통합, OOF 2,215
"""
import os
import numpy as np
import pandas as pd
...
print(f"\n  Stage 1 4모델 평균 OOF RMSE: {rmse_s1:,.0f}")

# ========================================
# Stage 2: PL(상위 50%) 추가 → 4모델 재학습 → 가중 평균
# ========================================
...
print(f"\n{'=' * 60}")
print(f"[Stage 2] PL2 + Scale Blend (상위 50% = {n_pseudo}건)")
print(f"{'=' * 60}")
...
# 가중 평균 (원본 스케일 60%)
W = {'cb_log': 0.20, 'cb_raw': 0.30, 'lgb_log': 0.20, 'lgb_raw': 0.30}

w_oof = sum(W[k] * oof_s2[k] for k in MODELS)
w_test = sum(W[k] * tpred_s2[k] for k in MODELS)
...
print(f"\n{'=' * 60}")
print(f"최종: {method} OOF RMSE {final_rmse:,.0f}")
print(f"전략 08: OOF 2,234 / Public 2,155")
print(f"{'=' * 60}")
