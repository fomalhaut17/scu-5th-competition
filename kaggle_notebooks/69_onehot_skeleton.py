"""
69 ONEHOT SKELETON + BLEND 56+63 = 60:40
Skeleton을 원-핫 인코딩 Gu/Dong으로 개선
전략66(라벨인코딩 Skeleton→2,048)에서 Skeleton 구조 변경
"""
import os
import numpy as np
import pandas as pd
...
ohe.fit(pd.concat([train_skel[['Gu', 'Dong']], test_skel[['Gu', 'Dong']]]))

num_cols_skel = [c for c in train_skel.columns if c not in ['Gu', 'Dong', 'Target']]
X_skel_raw = np.hstack([train_skel[num_cols_skel].values.astype(float), ohe.transform(train_skel[['Gu', 'Dong']])])
X_skel_test_raw = np.hstack([test_skel[num_cols_skel].values.astype(float), ohe.transform(test_skel[['Gu', 'Dong']])])
print(f"  Skeleton features: {X_skel_raw.shape[1]} (numeric {len(num_cols_skel)} + one-hot {X_skel_raw.shape[1]-len(num_cols_skel)})")

scaler_skel = StandardScaler()
X_skel_all = scaler_skel.fit_transform(X_skel_raw)
X_skel_test = scaler_skel.transform(X_skel_test_raw)
...
for alpha in [0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0]:
    ...
    for tr, va in kf_skel.split(X_skel_all):
        m = Ridge(alpha=alpha); m.fit(X_skel_all[tr], y_log[tr])
...
for tr, va in kf_skel.split(X_skel_all):
    m = Ridge(alpha=best_alpha_skel); m.fit(X_skel_all[tr], y_log[tr])
    skeleton_oof[va] = np.expm1(m.predict(X_skel_all[va]))
m_full = Ridge(alpha=best_alpha_skel); m_full.fit(X_skel_all, y_log)
...
print(f"  Skeleton OOF RMSE: {best_rmse_skel:,.0f} (alpha={best_alpha_skel})")
