"""
83 3-WAY BLEND v5b: Winsorized Residual (±2.5*std 클리핑)
전략81(pred_47 4시드, Public 2,028.01)에서 GBDT 잔차 학습 타깃을 Winsorize.
극단 잔차(Seongdong/Yongsan 등 고오차 구)가 잔차 GBDT를 왜곡하는 문제 완화.
OOT 빠른 테스트: raw 2,647.5 / ±2.0*std 2,642.3 / ±2.5*std 2,645.0. Public에서 ±2.5가 더 나을 가능성 테스트.
...
y_resid_pergu = np.clip(y_resid_pergu_raw, -2.5 * _std, 2.5 * _std).astype(float)
y_resid_up_pergu = (y_resid_pergu / area_train).astype(float)
print(f"  Winsorized Per-Gu residual: std={_std:.0f}, clip={2.5*_std:.0f}, clipped={np.sum(np.abs(y_resid_pergu_raw)>2.5*_std)}건")
