"""
62a GTR UNIT-PRICE TREND
GTR 변경: Target 평균 성장률 → Target/Area(평당가) 기준 성장률
...
# === GTR (Unit-Price Trend) ===
train_orig['_UnitPrice'] = train_orig['Target'] / train_orig['Exclusive_Area']
...
for gu in train_orig['Gu'].unique():
    monthly = train_orig[train_orig['Gu'] == gu].groupby('Transaction_YearMonth')['_UnitPrice'].mean()
    gu_growth[gu] = monthly.pct_change().dropna().mean()
train_orig = train_orig.drop(columns=['_UnitPrice'])
