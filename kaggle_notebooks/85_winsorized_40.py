"""
85 3-WAY BLEND v5e: Winsorized Residual ±4.0*std
전략84(±3.0*std, Public 2,024.88)에서 이어서 더 느슨한 ±4.0 테스트.
±2.0(2027.01) → ±2.5(2025.01) → ±3.0(2024.88)로 개선폭이 -1.00→-2.00→-0.13 급감 중.
±4.0에서 개선폭이 +0 또는 악화면 ±3.0이 최적점으로 확정, 계속 개선되면 더 느슨한 방향 탐색.
...
# Winsorize: 극단 잔차 클리핑 (±4.0*std). ±3.0(2024.88)에서 이어서 더 느슨하게 테스트.
_std = float(np.std(y_resid_pergu_raw))
y_resid_pergu = np.clip(y_resid_pergu_raw, -4.0 * _std, 4.0 * _std).astype(float)
y_resid_up_pergu = (y_resid_pergu / area_train).astype(float)
print(f"  Winsorized Per-Gu residual: std={_std:.0f}, clip={4.0*_std:.0f}, clipped={np.sum(np.abs(y_resid_pergu_raw)>4.0*_std)}건")
...
y_resid_oh = np.clip(y_resid_oh_raw, -4.0 * _std_oh, 4.0 * _std_oh).astype(float)
y_resid_up_oh = (y_resid_oh / area_train).astype(float)
print(f"  Winsorized One-Hot residual: std={_std_oh:.0f}, clipped={np.sum(np.abs(y_resid_oh_raw)>4.0*_std_oh)}건")
