"""
89 GEO COORDS: 전략84(Public 2,024.88, ±3.0 Winsorize + 50:30:20) 기반 + 동별 위경도 좌표 피처 추가.
지금까지 Gu/Dong은 전부 범주형(One-Hot/Label)으로만 다뤄져서 "인접 동은 가격도 비슷하다"는
공간적 연속성을 모델이 학습할 수 없었음. 24개 동의 대략적 중심 위경도를 새 수치형 피처로 추가해
GBDT가 지리적 그라디언트를 직접 학습하도록 구조적으로 바꿈 (실거래 데이터가 아닌 공개 행정동
지리정보라 [[project_synthetic_data]]의 "외부 실거래 매칭 불가"와는 무관, 안전).
좌표는 대략적 근사치(공개 지식 기반)이며 대규모 공간 그라디언트 포착이 목적이라 수백m 오차는 무방.
신규 피처라 OOT로도 검증 불가하다는 기존 교훈([[feedback_oot_before_submit]])에 따라 OOT 생략,
바로 Public 제출로 확인. 실패해도 최고점수 기준 리더보드라 순위 손해 없음(제출 슬롯만 소모).
"""
import os
import numpy as np
import pandas as pd
...
# 24개 동 중심 위경도 (근사치, 공개 행정동 지리정보 기반)
DONG_COORDS = {
    ('Eunpyeong-gu', 'Bulgwang'):    (37.6183, 126.9294),
    ('Eunpyeong-gu', 'Susaek'):      (37.5893, 126.9089),
    ('Eunpyeong-gu', 'Yeonsinnae'):  (37.6193, 126.9195),
    ('Gangnam-gu', 'Apgujeong'):     (37.5274, 127.0286),
    ('Gangnam-gu', 'Daechi'):        (37.4979, 127.0632),
    ('Gangnam-gu', 'Samsung'):       (37.5145, 127.0569),
    ('Mapo-gu', 'Ahyeon'):           (37.5577, 126.9564),
    ('Mapo-gu', 'Hapjeong'):         (37.5495, 126.9137),
    ('Mapo-gu', 'Sangam'):           (37.5794, 126.8896),
    ('Nowon-gu', 'Junggye'):         (37.6396, 127.0755),
    ('Nowon-gu', 'Sanggye'):         (37.6597, 127.0564),
    ('Nowon-gu', 'Wolgye'):          (37.6547, 127.0616),
    ('Seocho-gu', 'Bangbae'):        (37.4835, 126.9979),
    ('Seocho-gu', 'Banpo'):          (37.5040, 127.0126),
    ('Seocho-gu', 'Seocho'):         (37.4835, 127.0177),
    ('Seongdong-gu', 'Geumho'):      (37.5507, 127.0294),
    ('Seongdong-gu', 'Oksu'):        (37.5426, 127.0176),
    ('Seongdong-gu', 'Seongsu'):     (37.5445, 127.0559),
    ('Songpa-gu', 'Garak'):          (37.4929, 127.1183),
    ('Songpa-gu', 'Jamsil'):         (37.5133, 127.1000),
    ('Songpa-gu', 'Munjjeong'):      (37.4843, 127.1218),
    ('Yongsan-gu', 'Hannam'):        (37.5347, 127.0011),
    ('Yongsan-gu', 'Ichon'):         (37.5178, 126.9701),
    ('Yongsan-gu', 'Itaewon'):       (37.5347, 126.9947),
}
_SEOUL_CITY_HALL = (37.5665, 126.9780)
...
def base_preprocess(df):
    ...
    coords = [DONG_COORDS[(gu, dong)] for gu, dong in zip(df['Gu'], df['Dong'])]
    df['Lat'] = [c[0] for c in coords]
    df['Lon'] = [c[1] for c in coords]
    df['Dist_to_Center'] = np.sqrt((df['Lat'] - _SEOUL_CITY_HALL[0])**2 + (df['Lon'] - _SEOUL_CITY_HALL[1])**2)
