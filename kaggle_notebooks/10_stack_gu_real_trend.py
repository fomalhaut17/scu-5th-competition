"""
10 STACK GU REAL TREND
파이프라인: FE → CatBoost + LightGBM → Ridge 스태킹 → 구별 실제 트렌드 보정
변경점: Train 기반 추정 성장률 대신 외부 실거래 데이터의 실제 Q4→Q1 변화율 사용
"""
import os
import numpy as np
import pandas as pd
...
# === 구별 트렌드 보정 (외부 실거래 데이터 기반) ===
# 공공데이터포털 실거래가에서 계산한 2025 Q4 → 2026 Q1 실제 월성장률
ACTUAL_GU_MONTHLY_GROWTH = {
    'Eunpyeong-gu': -0.0118,
    'Gangnam-gu':   -0.0072,
    'Mapo-gu':      -0.0207,
    'Nowon-gu':     -0.0037,
...
trend_correction = np.array([
    (1 + ACTUAL_GU_MONTHLY_GROWTH[gu]) ** m
    for gu, m in zip(test['Gu'], months_ahead)
])

print("=== 구별 실제 월성장률 (외부 데이터 기반) ===")
for gu, g in sorted(ACTUAL_GU_MONTHLY_GROWTH.items(), key=lambda x: -x[1]):
...
# 구별 보정 효과
print(f"\n구별 보정 효과:")
for gu in sorted(ACTUAL_GU_MONTHLY_GROWTH.keys()):
    mask = test['Gu'] == gu
    before = stack_test_pred[mask].mean()
    after = final_pred[mask].mean()
    direction = "↑" if after > before else "↓"
    print(f"  {gu:15s}: {before:,.0f} → {after:,.0f} ({direction})")
