"""
09 STACK GU EWM TREND
파이프라인: FE → CatBoost + LightGBM → Ridge 스태킹 → 구별 지수가중 트렌드 보정
변경점: 구별 월성장률을 단순 평균 → 지수가중평균(EWM)으로 변경 (최근 월에 높은 가중치)
"""
import os
import numpy as np
import pandas as pd
...
# === 구별 트렌드 보정 계수 계산 (지수가중평균) ===
last_train_ym = train['Transaction_YearMonth'].max()
last_train_seq = (last_train_ym // 100 - 2024) * 12 + last_train_ym % 100

EWM_SPAN = 6  # 최근 6개월에 가중치 집중 (halflife ≈ 3.6개월)

gu_growth = {}
for gu in train['Gu'].unique():
    monthly = train[train['Gu'] == gu].groupby('Transaction_YearMonth')['Target'].mean().sort_index()
    pct = monthly.pct_change().dropna()
    if len(pct) >= 2:
        gu_growth[gu] = pct.ewm(span=EWM_SPAN).mean().iloc[-1]
    else:
        gu_growth[gu] = pct.mean() if len(pct) > 0 else 0.0
...
# ========================================
print("\n" + "=" * 50)
print("[3/3] Ridge 스태킹 + 구별 EWM 트렌드 보정")
print("=" * 50)
