"""
16 STACK GEO
파이프라인: FE + 지리 피처 → CatBoost + LightGBM → Ridge 스태킹 → 구별 트렌드 보정
변경점: 동별 좌표 기반 거리 피처 추가 (강남역, 시청, 한강까지 거리)
"""
import os
import numpy as np
import pandas as pd
...
# === 동별 좌표 (WGS84) ===
DONG_COORDS = {
    'Apgujeong':  (37.527, 127.028),
    'Daechi':     (37.494, 127.063),
    'Samsung':    (37.511, 127.059),
    'Bangbae':    (37.483, 126.981),
    'Banpo':      (37.505, 127.008),
    'Seocho':     (37.492, 127.007),
    'Garak':      (37.497, 127.118),
    'Jamsil':     (37.513, 127.100),
    'Munjjeong':  (37.485, 127.123),
    'Geumho':     (37.556, 127.018),
    'Oksu':       (37.543, 127.017),
    'Seongsu':    (37.544, 127.056),
    'Hannam':     (37.535, 127.003),
    'Ichon':      (37.522, 126.970),
    'Itaewon':    (37.534, 126.994),
    'Ahyeon':     (37.558, 126.957),
    'Hapjeong':   (37.550, 126.914),
    'Sangam':     (37.577, 126.890),
    'Junggye':    (37.643, 127.072),
    'Sanggye':    (37.653, 127.072),
    'Wolgye':     (37.632, 127.068),
    'Bulgwang':   (37.615, 126.928),
    'Susaek':     (37.584, 126.905),
    'Yeonsinnae': (37.619, 126.922),
}
...
train_p = add_geo_features(add_feature_engineering(base_preprocess(train)))
test_p = add_geo_features(add_feature_engineering(base_preprocess(test)))

geo_features = ['Dist_Gangnam', 'Dist_Seoul_Stn', 'Dist_Han_River', 'Latitude', 'Longitude']
print("=== 지리 피처 ===")
...
print(f"  전략 16 OOF: {stack_rmse:,.0f} ({stack_rmse - 2234:+,.0f})")
