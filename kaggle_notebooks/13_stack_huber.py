"""
13 STACK HUBER
파이프라인: FE → CatBoost(Huber) + LightGBM(Huber) → Ridge 스태킹 → 구별 트렌드 보정
변경점: RMSE → Huber Loss로 변경 (이상치에 강건한 학습)
"""
import os
import numpy as np
import pandas as pd
...
# Huber delta: log1p 스케일에서의 이상치 임계값
# log1p(Target) 기준 std ≈ 0.4 정도, delta=1.0은 약 2.5 std
HUBER_DELTA = 1.0
...
print("\n" + "=" * 50)
print("[1/3] CatBoost 학습 (Huber Loss)")
print("=" * 50)
for fold, (tr_idx, va_idx) in enumerate(kf.split(X_cb)):
    print(f"  Fold {fold+1}/{N_SPLITS}", end=" → ")
    m = CatBoostRegressor(loss_function=f'Huber:delta={HUBER_DELTA}', random_seed=42, verbose=0,
...
print(f"  OOF RMSE: {cb_rmse:,.0f} (전략08 CB: 2,264)")

print("\n" + "=" * 50)
print("[2/3] LightGBM 학습 (Huber Loss)")
print("=" * 50)
for fold, (tr_idx, va_idx) in enumerate(kf.split(X_lgb)):
    print(f"  Fold {fold+1}/{N_SPLITS}", end=" → ")
    m_lgb = lgb.LGBMRegressor(objective='huber', alpha=HUBER_DELTA, metric='rmse',
...
print(f"  OOF RMSE: {lgb_rmse:,.0f} (전략08 LGB: 2,289)")
...
print(f"  전략 13 OOF: {stack_rmse:,.0f} ({stack_rmse - 2234:+,.0f})")
