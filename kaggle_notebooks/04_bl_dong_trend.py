"""
04 BL DONG TREND
파이프라인: FE → CatBoost + LightGBM → 블렌딩 → 동별 트렌드 보정
변경점: 동(Dong)별 월성장률로 보정, 극단값은 구별 성장률로 대체
"""
import os
import numpy as np
import pandas as pd
...
dong_growth = {}
for dong in train['Dong'].unique():
    dong_data = train[train['Dong'] == dong]
    monthly = dong_data.groupby('Transaction_YearMonth')['Target'].mean()
    growth = monthly.pct_change().dropna().mean()
    gu = dong_data['Gu'].iloc[0]
    # 극단값(월 5% 초과) 또는 데이터 부족(20개월 미만)이면 구별 성장률 사용
    if growth > 0.05 or len(monthly) < 20:
        dong_growth[dong] = gu_growth[gu]
    else:
        dong_growth[dong] = growth

print("=== 동별 월성장률 (보정 후) ===")
for dong, g in sorted(dong_growth.items(), key=lambda x: -x[1]):
    print(f"  {dong:15s}: {g*100:+.2f}%")
...
trend_correction = np.array([(1 + dong_growth[dong]) ** m for dong, m in zip(test['Dong'], months_ahead)])
