"""
41 BLEND 28:26 = 70:30
파이프라인: 전략28 동일 + 최종 예측을 28(8모델 Ridge) 70% + 26상당(4모델 가중) 30% 블렌딩
목적: Private LB 방어용 Final 2 후보
"""
import os
import numpy as np
import pandas as pd
...
# 전략 28: 8모델 Ridge (최선)
best_key = min(
    {'8ridge': best_ridge_rmse, 'base_w': rmse_base_w, '8avg': rmse_8avg},
    key=lambda k: {'8ridge': best_ridge_rmse, 'base_w': rmse_base_w, '8avg': rmse_8avg}[k])

pred_28 = best_ridge_test  # 8모델 Ridge
pred_26 = 0.20*tpred_base['cb_log'] + 0.30*tpred_base['cb_raw'] + 0.20*tpred_base['lgb_log'] + 0.30*tpred_base['lgb_raw']  # 4모델 가중 (전략 26 상당)

# 블렌딩: 28(70%) + 26(30%)
BLEND_RATIO = 0.7
pred_blend = BLEND_RATIO * pred_28 + (1 - BLEND_RATIO) * pred_26

final_pred = pred_blend * trend_correction

print(f"\n{'=' * 60}")
print(f"최종: 28:26 = {BLEND_RATIO:.0%}:{1-BLEND_RATIO:.0%} 블렌딩")
print(f"{'=' * 60}")
print(f"  전략 28 (8모델 Ridge) OOF: {best_ridge_rmse:,.0f}")
print(f"  전략 26 (4모델 가중)  OOF: {rmse_base_w:,.0f}")
print(f"  블렌딩 전 평균: {pred_blend.mean():,.0f}")
print(f"  블렌딩 후(+GTR) 평균: {final_pred.mean():,.0f}")
