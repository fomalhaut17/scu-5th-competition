"""
07 BL3 GU TREND
파이프라인: FE → CatBoost + LightGBM + XGBoost → 3모델 블렌딩 → 구별 트렌드 보정
변경점: XGBoost 추가 (CB 55% + LGB 35% + XGB 10%)
"""
import os
import numpy as np
import pandas as pd
...
XGB_PARAMS = {
    'learning_rate': 0.019367112126401298,
    'max_depth': 3,
    'min_child_weight': 31,
    'subsample': 0.8193159914899073,
    'colsample_bytree': 0.5994209184499008,
    'reg_alpha': 0.00031593939933944514,
    'reg_lambda': 0.0004003258332069075,
    'gamma': 0.010494547980287117,
}

def xgb_model_fn():
    return xgb.XGBRegressor(
        objective='reg:squarederror', eval_metric='rmse',
        verbosity=0, random_state=42, n_estimators=3000, **XGB_PARAMS)

def xgb_fit_fn(model, X_tr, y_tr, X_va, y_va):
    model.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], verbose=False)
    return model

xgb_oof, xgb_test = kfold_train_predict(X_xgb, y_xgb, X_test_xgb, xgb_model_fn, xgb_fit_fn)

# ========================================
# 4. 3모델 블렌딩 + 구별 트렌드 보정
# ========================================
print("\n" + "=" * 50)
print("[4/4] 3모델 블렌딩 + 구별 트렌드 보정")
print("=" * 50)
...
for w0 in np.arange(0, 1.05, 0.05):
    for w1 in np.arange(0, 1.05 - w0, 0.05):
        ...
        if w2 < 0:
            continue
        pred = w0 * cb_oof + w1 * lgb_oof + w2 * xgb_oof
        rmse = np.sqrt(np.mean((pred - y_true) ** 2))
        if rmse < best_rmse:
            best_rmse = rmse
            best_weights = (w0, w1, w2)

print(f"CatBoost  가중치: {best_weights[0]:.0%}")
print(f"LightGBM 가중치: {best_weights[1]:.0%}")
print(f"XGBoost  가중치: {best_weights[2]:.0%}")
print(f"블렌딩 OOF RMSE: {best_rmse:,.0f}")

# 구별 트렌드 보정 적용
final_pred_raw = best_weights[0] * cb_test + best_weights[1] * lgb_test + best_weights[2] * xgb_test
