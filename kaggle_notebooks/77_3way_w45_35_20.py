"""
77 3-WAY BLEND v2: 56 (GBDT+GTR) + 63 (Per-Gu Skeleton) + 69 (One-Hot Skeleton), 가중치 45:35:20
전략75(50:30:20, Public 2,028.4)와 동급 OOT 점수(2,595.1)를 가진 다른 조합을 시도.
오늘 마지막 제출 슬롯 — 리더보드가 최고점수 기준이라 75의 2,028.4는 보존되고,
이 시도는 순수하게 "50:30:20 주변이 정말 평평한지" 추가 정보를 얻기 위한 것.
전략76(4-way, 새 컴포넌트 추가)과는 다름: 여기는 이미 Public 검증된 3개 컴포넌트의
재가중치일 뿐이라 75와 같은 종류의 안전한 시도.
"""
import os
import numpy as np
import pandas as pd
...
# -> 50:30:20(전략75, Public 2,028.4)과 45:35:20이 OOT 2,595.1로 동급 1~2위.
# 오늘 마지막 제출이라 75와 같은 안전한 종류(재가중치)의 다른 점을 시도.
w56, w63, w69 = 0.45, 0.35, 0.2
