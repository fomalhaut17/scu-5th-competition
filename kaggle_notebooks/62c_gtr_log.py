"""
62c GTR LOG TREND
GTR 변경: (1+g)^m → exp(log성장률 × m). 합성 DGP 곱셈구조에 적합
...
for gu in train_orig['Gu'].unique():
    ...
    gu_growth[gu] = log_monthly.diff().dropna().mean()
...
trend_correction = np.array([np.exp(gu_growth[gu] * m) for gu, m in zip(test_orig['Gu'], months_ahead)])
