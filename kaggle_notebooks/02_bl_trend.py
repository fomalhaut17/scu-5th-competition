"""
02 BL TREND
파이프라인: FE → CatBoost + LightGBM → 블렌딩 → 트렌드 보정
변경점: 학습 데이터의 월별 상승률(0.41%)로 2026년 예측값 보정
"""
import os
import numpy as np
import pandas as pd
...
# === 트렌드 보정 계수 계산 ===
monthly_avg = train.groupby('Transaction_YearMonth')['Target'].mean()
monthly_growth = monthly_avg.pct_change().dropna().mean()
last_train_ym = train['Transaction_YearMonth'].max()
last_train_seq = (last_train_ym // 100 - 2024) * 12 + last_train_ym % 100

print(f"월평균 성장률: {monthly_growth:.4f} ({monthly_growth*100:.2f}%)")
print(f"마지막 학습 월: {last_train_ym}")
...
# 테스트 데이터의 YearMonth_Seq (보정 계수 계산용)
...
trend_correction = (1 + monthly_growth) ** months_ahead
print(f"보정 범위: +{(trend_correction.min()-1)*100:.1f}% ~ +{(trend_correction.max()-1)*100:.1f}%")
