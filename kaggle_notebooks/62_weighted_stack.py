"""
47 LGB-ET
파이프라인: 전략45(10모델) + LGBM extra_trees(log/raw) 2모델 추가 → 12모델 Ridge → GTR
변경점: GB+랜덤분기 다양성 추가, OOF 2,191
"""
import os
import numpy as np
import pandas as pd
...
# === 샘플 가중치 계산 (Weighted Stacking용) ===
def calculate_weights(df):
    weights = np.ones(len(df))
    # 대형 평수 가중치
    weights[df['Exclusive_Area'] >= 120] *= 2.0
    # 취약 구 가중치
    target_gus = ['강남구', '성동구', '용산구']
    weights[df['Gu'].isin(target_gus)] *= 1.5
    # 가격 기반 가중치 (고가일수록 오차 절대값이 큼)
    weights *= (df['Target'] / df['Target'].mean())
    return weights

sample_weights = calculate_weights(train_orig).values
print(f"\n=== 샘플 가중치 통계 ===")
print(f"  Mean: {sample_weights.mean():.4f}, Max: {sample_weights.max():.4f}, Min: {sample_weights.min():.4f}")
...
# ========================================
# LGBM extra_trees 2모델 추가 (log/raw)
# ========================================
print(f"\n{'=' * 60}")
print("LGBM extra_trees 2모델 학습")
print("=" * 60)
...
print(f"  LGB-ET log OOF: {np.sqrt(np.mean((lgbet_oof['lgbet_log'] - y_true_orig)**2)):,.0f}")
print(f"  LGB-ET raw OOF: {np.sqrt(np.mean((lgbet_oof['lgbet_raw'] - y_true_orig)**2)):,.0f}")

# ========================================
# 12모델 Ridge 스태킹 (8 CB/LGB + 2 ET + 2 LGB-ET)
# ========================================
print(f"\n{'=' * 60}")
print("12모델 Ridge 스태킹")
print("=" * 60)
...
for alpha in [0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0]:
    ...
    print(f"  12모델 Ridge(α={alpha:5.1f}): OOF {ridge_rmse:,.0f}")
...
print(f"\n  ★ 최선: 12모델 Ridge(α={best_ridge_alpha}) → OOF {best_ridge_rmse:,.0f}")
print(f"  (전략 45 기준: OOF 2,193)")

print(f"\n{'=' * 60}")
print(f"최종: 12모델 Ridge(α={best_ridge_alpha}) OOF {best_ridge_rmse:,.0f}")
print(f"{'=' * 60}")
