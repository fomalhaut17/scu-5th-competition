"""
87 RATIO RESWEEP: 56:63:69 = 40:30:30 (전략84 ±3.0 Winsorize 구조 기준 OOT 재탐색)
전략75의 50:30:20은 Winsorize 이전 버전 기준 OOT 탐색이었음. 전략86(로컬 OOT, holdout 3개월)에서
±3.0 Winsorize 구조로 231개 비율 조합을 재탐색한 결과 40:30:30이 최적(2,588.8, 기존 50:30:20은
2,590으로 8위/231, 차이 1.2점). 사니티체크(56단독>60:40>50:30:20 재현) 통과했으나 상위 15개 조합이
2,588.8~2,590.4 사이로 평평해 유의미한 개선인지는 불확실 — Public으로 확인 필요.
"""
import os
import numpy as np
import pandas as pd
...
# 전략86: ±3.0 Winsorize 구조 기준 OOT(holdout 3개월) 재탐색. 56단독 2,628 -> 60:40 2,598
# (기존 사실 재현, 사니티체크 통과) -> 231개 조합 그리드서치 결과 40:30:30이 2,588.8로 최적.
# 기존 50:30:20(2,590)과 1.2점 차이로 그 구간 자체가 평평함 (전략77 45:35:20 선례와 동일 패턴).
w56, w63, w69 = 0.4, 0.3, 0.3
