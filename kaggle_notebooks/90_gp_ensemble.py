"""
90 GP ENSEMBLE: 전략84(Public 2,024.88) 기반 + Gaussian Process Regression을 pred_56 스택에
신규 멤버로 추가 (12모델 -> 14모델). 지금까지 시도한 모델은 전부 트리 계열(CB/LGB/ET) +
선형(Ridge) 조합뿐이라, 완전히 다른 함수족인 GP(스무스한 커널 기반 회귀)로 진짜 다양성을 노림.
GP는 트리와 달리 각도가 매끄러운 함수를 학습해 트리가 못 잡는 보간적 구조를 잡을 가능성 있음.
BLEND_W53=0.0이 이미 확정된 사실이라(전략80) PART1(pred_53) 계산은 제거해 그만큼 연산량을 절감.
pred_63/69(Skeleton+Residual)는 건드리지 않고 pred_56 스택에만 GP를 추가.
신규 컴포넌트라 OOT 검증 불가([[feedback_oot_before_submit]]) — 사용자 요청으로 바로 Public 확인.
"""
import os
import numpy as np
import pandas as pd
...
GP_KERNEL = ConstantKernel(1.0, (1e-2, 1e2)) * RBF(length_scale=1.0, length_scale_bounds=(1e-2, 1e2)) \
...
def train_12models(X_cb, X_test_cb, X_lgb, X_test_lgb, cat_idx,
    ...
    for fold, (tr, va) in enumerate(kf.split(X_cb)):
        ...
        m = GaussianProcessRegressor(kernel=GP_KERNEL, alpha=1e-6, normalize_y=True, random_state=seed)
        m.fit(X_sc_tr, y_log[tr] if isinstance(y_log, np.ndarray) else y_log.iloc[tr])
        all_oof.setdefault('gp_log', np.zeros(len(X_cb)))[va] = np.expm1(m.predict(X_sc_va))
        all_tpred['gp_log'] = all_tpred.get('gp_log', np.zeros(len(X_test_cb))) + np.expm1(m.predict(X_sc_te)) / 5

        m = GaussianProcessRegressor(kernel=GP_KERNEL, alpha=1e-6, normalize_y=True, random_state=seed)
        m.fit(X_sc_tr, y_raw[tr])
        all_oof.setdefault('gp_raw', np.zeros(len(X_cb)))[va] = m.predict(X_sc_va)
        all_tpred['gp_raw'] = all_tpred.get('gp_raw', np.zeros(len(X_test_cb))) + m.predict(X_sc_te) / 5
...
# =============================================
# 전략 47 (PL2 + 14모델, GP 포함) — BLEND_W53=0.0 확정 반영, PART1(pred_53) 계산 생략
# =============================================
