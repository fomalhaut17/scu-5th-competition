"""
82 3-WAY BLEND v5: Winsorized Residual (±2.0*std 클리핑)
전략81(pred_47 4시드, Public 2,028.01)에서 GBDT 잔차 학습 타깃을 Winsorize.
극단 잔차(Seongdong/Yongsan 등 고오차 구)가 잔차 GBDT를 왜곡하는 문제 완화.
OOT 빠른 테스트: skeleton+1시드 잔차 기준 raw 2,647.5 → winsorized 2,642.3 (±2.0*std).
...
y_resid_oh = np.clip(y_resid_oh_raw, -2.0 * _std_oh, 2.0 * _std_oh).astype(float)
y_resid_up_oh = (y_resid_oh / area_train).astype(float)
print(f"  Winsorized One-Hot residual: std={_std_oh:.0f}, clipped={np.sum(np.abs(y_resid_oh_raw)>2.0*_std_oh)}건")
