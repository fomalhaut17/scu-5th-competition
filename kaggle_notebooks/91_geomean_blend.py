"""
91 GEOMEAN BLEND: 전략84(Public 2,024.88)와 동일한 12모델/3-way 구조에서
최종 블렌딩 연산자만 산술평균 -> 로그공간 가중 기하평균으로 교체.
final = exp(0.5*ln(p56) + 0.3*ln(p63) + 0.2*ln(p69)). 가격은 로그정규에 가까워
기하평균이 이론적으로 더 자연스럽고, 예측값 분산이 큰 구간에서 소폭 보수적(하향)으로 작동.
새 컴포넌트/피처/모델이 전혀 없는 순수 결합 방식 변경 — 지금까지 안 건드린 축.
BLEND_W53=0.0 확정(전략80)이라 PART1(pred_53) 계산은 제거(수학적으로 동일, 런타임 절반).
"""
import os
import numpy as np
import pandas as pd
