...
──────────────────────────
축약명  : PL SWEEP
주요 전략: 전략47 파이프라인 그대로, PL2 confidence percentile을 30~70% 탐색
차별점  : 지금까지 50%로 고정. 품질/양 균형점이 최적인지 확인
"""
import sys, os
...
# ========================================
# Stage 1: PL2 신뢰도 (1회만 실행)
# ========================================
print("=" * 60)
print("[Stage 1] PL2 신뢰도 계산 (공통)")
print("=" * 60)
...
print(f"\n  신뢰도 분포: min={confidence.min():.4f}, p25={np.percentile(confidence,25):.4f}, "
      f"p50={np.percentile(confidence,50):.4f}, p75={np.percentile(confidence,75):.4f}, max={confidence.max():.4f}")

# ========================================
# Stage 2: 임계값별 12모델 학습
# ========================================
THRESHOLDS = [0, 30, 40, 50, 60, 70, 100]

results = {}

for pct in THRESHOLDS:
    if pct == 0:
        mask_pl = np.zeros(len(test_orig), dtype=bool)
        n_pl = 0
    elif pct == 100:
        mask_pl = np.ones(len(test_orig), dtype=bool)
        n_pl = len(test_orig)
    else:
        thr = np.percentile(confidence, 100 - pct)
        mask_pl = confidence >= thr
        n_pl = mask_pl.sum()

    print(f"\n{'=' * 60}")
    print(f"[PL {pct}%] 채택 {n_pl}건 (Train {n_orig} + PL {n_pl} = {n_orig + n_pl}건)")
    print("=" * 60)
    ...
    for alpha in [0.1, 0.5, 1.0, 5.0, 10.0, 50.0]:
        ...
        if rmse < best_alpha_rmse:
            ...
            best_alpha_test = s_test.copy()
            best_alpha_val = alpha

    results[pct] = {
        'rmse': best_alpha_rmse,
        'alpha': best_alpha_val,
        'test_pred': best_alpha_test,
        'n_pl': n_pl,
    }
    print(f"  → 12모델 Ridge(α={best_alpha_val}): OOF {best_alpha_rmse:,.0f}")

# ========================================
# 결과 요약
# ========================================
print(f"\n{'=' * 60}")
print("PL Threshold Sweep 결과")
print("=" * 60)

baseline_rmse = results[50]['rmse']
best_pct = None
best_rmse = float('inf')
...
for pct in THRESHOLDS:
    ...
    if r['rmse'] < best_rmse:
        best_rmse = r['rmse']
        best_pct = pct
    print(f"  PL {pct:3d}% ({r['n_pl']:3d}건) α={r['alpha']:4.1f}: OOF {r['rmse']:,.0f} ({diff:+,.0f} vs 50%){marker}")

print(f"\n  ★ 최선: PL {best_pct}% → OOF {best_rmse:,.0f}")
print(f"  (전략 47 = PL 50%: OOF {baseline_rmse:,.0f})")

# 최선 제출 파일
final_pred = results[best_pct]['test_pred'] * trend_correction
sub = sample_sub.copy()
sub['Target'] = final_pred
sub.to_csv(os.path.join(os.path.dirname(__file__), '..', '..', f'submission_l4_49_pl{best_pct}.csv'), index=False)
print(f"\n제출 파일 생성: submission_l4_49_pl{best_pct}.csv")

# 50%가 아닌 경우만 개선 기록
if best_pct != 50:
    record_result('L4', 49, 'PL SWEEP', f'PL {best_pct}% 12모델 Ridge(α={results[best_pct]["alpha"]}) + GTR', best_rmse, 'tested')
else:
    record_result('L4', 49, 'PL SWEEP', f'PL threshold sweep: 50%가 최적 확인', baseline_rmse, 'tested')
