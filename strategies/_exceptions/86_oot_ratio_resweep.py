"""
86 OOT RATIO RESWEEP: 56:63:69 외부 블렌딩 비율을 ±3.0*std Winsorized 잔차 구조로 재탐색.
전략75에서 50:30:20을 찾은 OOT 스윕은 Winsorize 이전(전략56/63/69 원본) 버전 기준이었음.
전략82~85로 63/69의 잔차 학습 타깃이 ±3.0*std Winsorize로 바뀌었으므로,
같은 컴포넌트라도 내부 구조가 달라져 최적 외부 가중치가 바뀔 수 있음 -> OOT로 재확인.

방법: utils.oot_split(holdout_months=3)로 train_orig를 시간분리 (real label 있는 미래 3개월).
oot_train(과거)만으로 56/63/69를 학습해서 oot_val(미래)을 예측 -> 실제 RMSE로 비교.
BLEND_W53=0.0 (전략80~ 확정)이므로 pred_56 = pred_47(PL2)뿐, PART1(pred_53)은 스킵해 연산량 절감.
Public 제출 없이 로컬에서만 실행 (제출 슬롯 소모 없음).
"""
import os
import sys
import time
...
test_orig_labeled = train_full.loc[va_idx].reset_index(drop=True)  # "test" = OOT holdout, 실제 라벨 있음
y_true_orig = train_orig['Target'].values
y_true_val = test_orig_labeled['Target'].values
test_orig = test_orig_labeled.drop(columns=['Target'])  # 실제 test_orig.csv처럼 Target 없이 피처만
...
# =============================================
# 전략 47 (PL2 + 12모델, 4시드) -> pred_56 = pred_47 * GTR (BLEND_W53=0.0 확정 반영, PART1 스킵)
# =============================================
...
for seed47 in SEEDS:
    ...
    kf_meta = KFold(n_splits=N_SPLITS, shuffle=True, random_state=42)
    pred_47_s, rmse_47_s, oof_47_pred_s = ridge_stack(oof_47_s, tpred_47_s, y_true_orig, n_orig, len(test_orig), kf_meta)
    print(f"    Seed {seed47}: OOF {rmse_47_s:,.0f}")
    seed_tests_47.append(pred_47_s)
    seed_oofs_47.append(oof_47_pred_s)

pred_47 = np.mean(seed_tests_47, axis=0)

pred_56 = pred_47 * trend_correction
val_rmse_56 = np.sqrt(np.mean((pred_56 - y_true_val) ** 2))
print(f"\n  전략56(=47×GTR) OOT-val RMSE: {val_rmse_56:,.0f}")

# =============================================
# 전략 63: Per-Gu Ridge Skeleton + Winsorized(±3.0) GBDT Residual
# =============================================
...
print(f"  Winsorized Per-Gu residual(±3.0std): std={_std:.0f}, clipped={np.sum(np.abs(y_resid_pergu_raw)>3.0*_std)}건")
...
val_rmse_63 = np.sqrt(np.mean((pred_63 - y_true_val) ** 2))
print(f"  전략63 OOT-val RMSE: {val_rmse_63:,.0f}")

# =============================================
# 전략 69: One-Hot Gu/Dong Skeleton + Winsorized(±3.0) GBDT Residual
# =============================================
...
print(f"  Winsorized One-Hot residual(±3.0std): std={_std_oh:.0f}, clipped={np.sum(np.abs(y_resid_oh_raw)>3.0*_std_oh)}건")
...
val_rmse_69 = np.sqrt(np.mean((pred_69 - y_true_val) ** 2))
print(f"  전략69 OOT-val RMSE: {val_rmse_69:,.0f}")

# =============================================
# 비율 그리드서치 (실제 val 라벨 기준 RMSE, naive OOF 아님)
# =============================================
print(f"\n{'=' * 60}\n비율 그리드서치 (OOT-val 실측 RMSE)  ({time.time()-t0:,.0f}s)\n{'=' * 60}")

print(f"  56단독: {val_rmse_56:,.0f} / 63단독: {val_rmse_63:,.0f} / 69단독: {val_rmse_69:,.0f}")

# 기존 확정 사실 재현 여부(사니티체크): 56단독 > 60:40(56+63) > 50:30:20(3-way)
baseline_6040 = np.sqrt(np.mean((0.6 * pred_56 + 0.4 * pred_63 - y_true_val) ** 2))
baseline_503020 = np.sqrt(np.mean((0.5 * pred_56 + 0.3 * pred_63 + 0.2 * pred_69 - y_true_val) ** 2))
print(f"  [사니티체크] 56단독 {val_rmse_56:,.0f} vs 60:40 {baseline_6040:,.0f} vs 기존 50:30:20 {baseline_503020:,.0f}")
...
for w56 in grid:
    for w63 in grid:
        w69 = round(1.0 - w56 - w63, 2)
        if w69 < 0 or w69 > 1.0:
            continue
        pred_mix = w56 * pred_56 + w63 * pred_63 + w69 * pred_69
        rmse = np.sqrt(np.mean((pred_mix - y_true_val) ** 2))
        results.append((w56, w63, w69, rmse))

results.sort(key=lambda x: x[3])
print(f"\n  상위 15개 조합 (OOT-val RMSE 기준):")
for w56, w63, w69, rmse in results[:15]:
    print(f"    56×{w56:.0%} + 63×{w63:.0%} + 69×{w69:.0%} = {rmse:,.1f}")

print(f"\n  기존 50:30:20 순위: {[i for i, r in enumerate(results) if r[0]==0.5 and r[1]==0.3][0]+1}위 / {len(results)}개 조합")
