...
──────────────────────────
축약명  : ALGO DIV V2
주요 전략: 전략45(8+ET2=10모델)에 RandomForest/XGBoost/ET변형 추가 → 최대 다양성 탐색
차별점  : ET가 Public -2 확인, 같은 원리로 더 확장
"""
import sys, os
...
for fold, (tr, va) in enumerate(kf2.split(X_cb2)):
    ...
    # --- ExtraTrees 변형 (다른 depth/seed) ---
    m = ExtraTreesRegressor(n_estimators=500, max_depth=8, min_samples_leaf=15, random_state=123, n_jobs=-1)
    m.fit(X_sc_tr, y_log_aug.iloc[tr])
    all_oof.setdefault('et2_log', np.zeros(len(X_cb2)))[va] = np.expm1(m.predict(X_sc_va))
    all_tpred['et2_log'] = all_tpred.get('et2_log', np.zeros(len(X_test_cb2))) + np.expm1(m.predict(X_sc_te)) / 5

    m = ExtraTreesRegressor(n_estimators=500, max_depth=8, min_samples_leaf=15, random_state=123, n_jobs=-1)
    m.fit(X_sc_tr, y_raw_aug[tr])
    all_oof.setdefault('et2_raw', np.zeros(len(X_cb2)))[va] = m.predict(X_sc_va)
    all_tpred['et2_raw'] = all_tpred.get('et2_raw', np.zeros(len(X_test_cb2))) + m.predict(X_sc_te) / 5

    # --- RandomForest ---
    m = RandomForestRegressor(n_estimators=500, max_depth=12, min_samples_leaf=10, random_state=42, n_jobs=-1)
    m.fit(X_sc_tr, y_log_aug.iloc[tr])
    all_oof.setdefault('rf_log', np.zeros(len(X_cb2)))[va] = np.expm1(m.predict(X_sc_va))
    all_tpred['rf_log'] = all_tpred.get('rf_log', np.zeros(len(X_test_cb2))) + np.expm1(m.predict(X_sc_te)) / 5

    m = RandomForestRegressor(n_estimators=500, max_depth=12, min_samples_leaf=10, random_state=42, n_jobs=-1)
    m.fit(X_sc_tr, y_raw_aug[tr])
    all_oof.setdefault('rf_raw', np.zeros(len(X_cb2)))[va] = m.predict(X_sc_va)
    all_tpred['rf_raw'] = all_tpred.get('rf_raw', np.zeros(len(X_test_cb2))) + m.predict(X_sc_te) / 5

    # --- XGBoost ---
    m = xgb.XGBRegressor(n_estimators=3000, max_depth=3, learning_rate=0.01,
                          subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
                          random_state=42, verbosity=0, early_stopping_rounds=100)
    m.fit(X_cb2.iloc[tr], y_log_aug.iloc[tr], eval_set=[(X_cb2.iloc[va], y_log_aug.iloc[va])], verbose=False)
    all_oof.setdefault('xgb_log', np.zeros(len(X_cb2)))[va] = np.expm1(m.predict(X_cb2.iloc[va]))
    all_tpred['xgb_log'] = all_tpred.get('xgb_log', np.zeros(len(X_test_cb2))) + np.expm1(m.predict(X_test_cb2)) / 5

    m = xgb.XGBRegressor(n_estimators=3000, max_depth=3, learning_rate=0.01,
                          subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
                          random_state=42, verbosity=0, early_stopping_rounds=100)
    m.fit(X_cb2.iloc[tr], y_raw_aug[tr], eval_set=[(X_cb2.iloc[va], y_raw_aug[va])], verbose=False)
    all_oof.setdefault('xgb_raw', np.zeros(len(X_cb2)))[va] = m.predict(X_cb2.iloc[va])
    all_tpred['xgb_raw'] = all_tpred.get('xgb_raw', np.zeros(len(X_test_cb2))) + m.predict(X_test_cb2) / 5
...
# ========================================
print(f"\n{'=' * 60}")
print("개별 모델 OOF + 기존 8모델 평균과의 상관")
print("=" * 60)

base_8 = ['cb_log', 'cb_raw', 'lgb_log', 'lgb_raw', 'cb_up_log', 'cb_up_raw', 'lgb_up_log', 'lgb_up_raw']
existing_avg = np.mean([all_oof[k] for k in base_8], axis=0)

for k in sorted(all_oof.keys()):
    r = np.sqrt(np.mean((all_oof[k] - y_true_orig)**2))
    corr = np.corrcoef(existing_avg, all_oof[k])[0, 1]
    tag = " [기존]" if k in base_8 else " [NEW]"
...
# 기준
_, _, rmse_8 = ridge_stack(base_8, 10.0)
print(f"\n  기준 8모델 Ridge(α=10): OOF {rmse_8:,.0f}")

# 전략 45 (8+ET2)
et2 = ['et_log', 'et_raw']
_, _, rmse_10 = ridge_stack(base_8 + et2, 10.0)
print(f"  전략45 (8+ET2):         OOF {rmse_10:,.0f} ({rmse_10-rmse_8:+,.0f})")

# 새 조합들
combos = {
    "8+ET2+ET변형2": base_8 + et2 + ['et2_log', 'et2_raw'],
    "8+ET2+RF2": base_8 + et2 + ['rf_log', 'rf_raw'],
    "8+ET2+XGB2": base_8 + et2 + ['xgb_log', 'xgb_raw'],
    "8+ET2+RF2+XGB2": base_8 + et2 + ['rf_log', 'rf_raw', 'xgb_log', 'xgb_raw'],
    "8+ET4+RF2+XGB2": base_8 + et2 + ['et2_log', 'et2_raw', 'rf_log', 'rf_raw', 'xgb_log', 'xgb_raw'],
    "8+RF2": base_8 + ['rf_log', 'rf_raw'],
    "8+XGB2": base_8 + ['xgb_log', 'xgb_raw'],
}

best_rmse = rmse_10
best_test = None
best_name = "전략45 (8+ET2)"
...
for name, keys in combos.items():
    for alpha in [10.0, 50.0]:
        ...
        if r < best_rmse:
            ...
            marker = " ★"
        print(f"  {name:25s} α={alpha:4.0f}: OOF {r:,.0f} ({diff:+,.0f}){marker}")

print(f"\n  ★ 최선: {best_name} → OOF {best_rmse:,.0f}")
print(f"  (전략 28: {rmse_8:,.0f}, 전략 45: {rmse_10:,.0f})")

# 제출 파일
if best_test is not None:
    final_pred = best_test * trend_correction
else:
    _, best_test, _ = ridge_stack(base_8 + et2, 10.0)
    final_pred = best_test * trend_correction

sub = sample_sub.copy()
sub['Target'] = final_pred
sub.to_csv(os.path.join(os.path.dirname(__file__), '..', '..', 'submission_l4_46_algo_div.csv'), index=False)
print(f"\n제출 파일 생성: submission_l4_46_algo_div.csv")

record_result('L4', 46, 'ALGO DIV V2', f'알고리즘 다양성 v2 ({best_name}) + GTR', best_rmse, 'tested')
