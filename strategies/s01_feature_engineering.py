...
축약명  : FE
구성    : 단독 전략
주요 전략: baseline 피처에 교차항/비율 피처 4개 추가
차별점  : YearMonth_Seq, Area_x_Floor, Floor_per_Area, Brand_x_Area
결과    : OOF RMSE 2,403 (baseline 대비 -384, -13.8%)
제출파일: submission_s01_fe.csv
"""
import sys, os
...
from utils import load_data, base_preprocess, add_feature_engineering, encode_categoricals, kfold_train_predict, save_submission
...
print("[S01-FE] 5-Fold CV")
oof_pred, test_pred, rmse = kfold_train_predict(X, y, X_test, model_fn, fit_fn)

save_submission(sample_sub, test_pred, 'submission_s01_fe.csv')
